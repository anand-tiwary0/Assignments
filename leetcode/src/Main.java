...
class Solution {
    public boolean search(int[] arr, int target) {
        ...
        while(start<=end){
            ...
            //in case all three are equal and we can't decide which part is sorted
            if(arr[start] == arr[mid] && arr[end]==arr[mid])  //and this condition will hit only when target is not equal to any of them so just negelect them to reduce search space until we get exactly one sorted part to implement binary search logic
            {
                start = start + 1;
                end = end -1;
                continue;
...
class Solution {
    public int findPeakElement(int[] nums) {
        ...
        if(nums[lastindex]>nums[lastindex-1]) return lastindex;
        int start = 1;
        int end = lastindex - 1;
        while(start<=end){
            int mid = start + (end - start)/2;
            if(nums[mid]>nums[mid-1] && nums[mid]>nums[mid+1]) return mid;
            //lets decide on which slop we are on
...
class Solution {
    ...
    public static int search(int[] nums,int target, boolean which){
        int start = 0;
        int end = nums.length - 1;
        int ansindex = -1;
        while(start<=end){
